...
## 本番デプロイ（インターネット越しに対戦する）

ローカルネットワークを越えて離れた相手と対戦するには、サーバー・クライアントをホスティングサービスへデプロイする必要がある。Render（無料枠あり）へのデプロイ手順は [docs/deploy-render.md](docs/deploy-render.md) を参照。
